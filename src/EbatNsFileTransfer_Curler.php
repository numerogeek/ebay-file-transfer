...
class EbatNsFileTransfer_Curler
{
    ...
    private $methodHeaderName = null; // 'X-EBAY-SOA-OPERATION-NAME', 'X-EBAY-API-CALL-NAME'
    private $serviceHeaderName = null; // 'X-EBAY-SOA-OPERATION-NAME', 'X-EBAY-API-CALL-NAME'
    ...
    public function request($xml = '', $priorHeaders = [])
    {
        ...
        if (!$this->multipartContent) {
            ...
            $this->addXmlToMultipartRequest($xml);
            $xml = $this->multipartContent;
    ...
    public function parse($responseParser = null)
    {
        ...
        if ($results && $results[0]->getContent() instanceof EbatNsFileTransfer_ResponseError) {
        } elseif (!$handles) {
        } elseif (!$mh) {
            $ch = $handles[0];
            $raw = curl_exec($ch);
            ...
            if ($raw !== false && $raw !== '') {
                $results[] = EbatNsFileTransfer_Decoder::decode($raw, $parser);
            } else {
                $message = $errorNumber ? "curl: $errorNumber " . curl_error($ch) : 'Empty response';
                $error = new EbatNsFileTransfer_ResponseError($message);
            }
        } else {
            do {
                ...
                if ($status !== CURLM_OK) {
                    $error = new EbatNsFileTransfer_ResponseError('curl(m): ' . curl_multi_strerror($status));
                    $handles = [];
                    break;
                }
            } while ($running);
        ...
        if ($error) {
            $results[] = new EbatNsFileTransfer_DecodedResponse($error);
        }
    ...
    private function parseFromCurlBlockedApi($parser)
    {
        ...
        foreach ($this->customHandles as $ch) {
            $raw = '';
            $host = parse_url($ch->url, PHP_URL_HOST);
            ...
            if ($fp === false) {
                $results[] = new EbatNsFileTransfer_ResponseError("Problem with socket-connect ($host)");
                break;
            }
            ...
            if ($transferEncoding) {
                $error = new EbatNsFileTransfer_ResponseError("Transfer encoding not supported: $transferEncoding, Size: $contentLength");
                break;
            }
            ...
            if ($raw === '') {
                $error = new EbatNsFileTransfer_ResponseError('Empty response from: ' . $ch->url . ' ' . $ch->headers[0]);
                break;
            }

            $results[] = EbatNsFileTransfer_Decoder::decode("$headerText\r\n$raw", $parser);
        }
    ...
    public function withMultipartOctetStream($data, $name, $filename)
    {
        if ($data) {
    ...
    private function mergeHeaders($headers = [])
    {
        ...
        foreach ([$this->headers, $headers] as $arr) {
            foreach ($arr as $key => $val) {
                ...
                $value = implode(': ', $nokey ? $parts : [$key, $val]);
