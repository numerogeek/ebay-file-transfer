...
class EbatNsFileTransfer_Decoder
{
    public static function decode($raw, $parser = null)
    {
        ...
        if (count($match) !== 2) {
            return new EbatNsFileTransfer_DecodedResponse($raw);
        }
        ...
        $result = self::parseContent(new EbatNsFileTransfer_DecodedResponse($isMultipart ? null : $raw, $assocHeaders), $parser);
        ...
        while ($eof !== true) {
            ...
            if (count($match) === 2) {
                ...
                $file = new EbatNsFileTransfer_DecodedResponse($body, self::parseHeaders($header));
        ...
        if (!$result->getContent() && !$result->getFiles()) {
            $result->setContent(new EbatNsFileTransfer_ResponseError('Was not able to decode the response!'));
        }
