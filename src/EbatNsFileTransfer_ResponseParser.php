...
class EbatNsFileTransfer_ResponseParser
{
    protected $typeNs;
    ...
    public function __construct($dataConverter, $callbacks = [], $typeNs = 'http://www.ebay.com/marketplace/services')
    {
    ...
    public function parse($xml, $tag = null, $type = null, $errorParsingEnabled = true)
    {
        ...
        if ($xmlParserError) {
            return new EbatNsFileTransfer_ResponseError($xmlParserError, 90000 + 1);
        }
        ...
        if (!$errorParsingEnabled) {
            return new EbatNsFileTransfer_ResponseError($xml, 90000 + 11);
        }

        return EbatNsFileTransfer_ResponseError::parseError($xml, $this);
    }
    ...
    protected function _startElement($parser, $prefixedName, $attrs)
    {
        ...
        if (strtolower($prefixedName) === 'soapenv:fault') { // fault found before in response
            ...
            $tag = $name;
            $parent = end($this->map);
            $info = $parent->cls::readElement($tag);
            $nsTypeName = self::resolveTypeClass($info === null ? $tag : $info['type']);
            $isEnum = $nsTypeName && is_subclass_of($nsTypeName,'InTradeSys\eBay\fileTransfer\EbatNsFileTransfer_EnumType');
            ...
            if ($isAcceptedNs && !$isEnum) {
                ...
                foreach ($attrs as $key => $value) {
                    $cls->setTypeAttribute($key, $value);
                }
            }
    ...
    {
        $base = strpos($name, '\\') ? $name : 'InTradeSys\eBay\fileTransfer\\' . ucfirst($name);
    ...
    {
        $response = new EbatNsFileTransfer_ResponseError();
        $code = '';
        $severity = '';
